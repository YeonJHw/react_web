...
public class JwtConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		return http.httpBasic().disable()	//HTTP 기본인증(사용자이름/비밀번호로 인증을 사용하지 않겠다)
				.csrf().disable().cors()	//filter에서 따로 검증하기 때문에 csrf 공격을 비활성화
				.and()						
				.authorizeRequests()		//요청에 대한 권한 설정
				//post 요청중에 /member/login, /member/join은 그냥 허용
				.antMatchers(HttpMethod.POST, "/member/login","/member/join").permitAll()
				//post 요청중에 /member/로 시작하면 바늗시 인증을 하도록 설정
